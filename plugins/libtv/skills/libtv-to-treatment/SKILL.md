---
name: libtv-to-treatment
description: 将用户指定 LibTV 画布中的已有图片、视频、音频、剧本、分镜和可访问的创作对话，整理成中文为主、16:9 横版、可翻页演示的导演提案 HTML。适用于 MV Treatment、画布成果提案及导演阐述；优先按画布名收集已完成作品；用户没有现成作品时，新建 LibTV 画布并默认衔接 POP MV 自动生成，再整理提案。单纯生成媒体或编辑画布时不使用本技能。
---
...
优先询问：“请提供一个 LibTV 站内已经完成作品的画布名称；如果还没有完成的作品，我会帮你新建画布，默认使用 POP MV 自动生成，再整理成 Treatment。”若用户已提供画布名、链接或明确的项目 ID，直接定位，不重复询问。Studio 信息可在同次问询中收集，但不能抢在作品入口之前成为阻塞步骤。

先使用 MCP 连接返回的 instructions 与 `get_capabilities` 验证 LibTV 工具来源与运行状态。本文中的工具名是发现线索；实际参数以当前工具 schema 为准，不猜接口、模型 ID 或私有地址。当前仅要求编辑本 skill 时，只更新规则，不执行新建画布或媒体生成。
...
用户表明没有现成作品并进入本路线后，帮助其在 LibTV 新建画布，默认使用 **POP MV skill** 自动制作。若用户只咨询、只要规划或明确禁止生成，遵循该限制；不能把未回复第一步问询当作进入生成路线。

1. 先通过当前 LibTV 技能发现接口定位真实可用的 POP MV，解析其准确标识与要求。POP MV 是此流程由用户指定的默认选择；沿用该选择并通过原生 Skill 选择字段传递，不重复让用户从泛用技能列表选择。不要假设它有固定 ID，不能因不可用而静默改用别的创作技能。
...
用户请求本地 HTML 素材包已包含下载其所选画布相关媒体的任务范围。按 LibTV 媒体下载路线，将入选资源保存至明确本地目录。保留源文件，网页压缩副本另存；不扫描无关本地文件。
...
工具能力和认证方式使用当前 LibTV 基础技能与实时 schema，不在此固化私有端点、临时资源地址或特定账号信息。
