...
- If the in-app browser cannot deliver a verified WAV, reuse a task-owned Chrome piano tab and record/export there. Keep the LibTV canvas open in the in-app browser. Do not reuse another task's existing recording or assume identical URLs identify the same piano tab.
