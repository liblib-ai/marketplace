...
3. Preview the selected window. Crop the browser chrome if helpful, while retaining the complete piano keyboard and visible recording controls. Use current window dimensions and page bounds, not saved pixel coordinates. Keep the LibTV canvas tab open in its agreed panel.
